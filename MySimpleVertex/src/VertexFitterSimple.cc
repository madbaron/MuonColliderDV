...
VertexingUtils::FCCAnalysesVertex VertexFitterSimple::VertexFitter_Tk(int Primary,
                                                                      ROOT::VecOps::RVec<lcio::TrackImpl> tracks,
                                                                      ...
                                                                      double bsc_x, double bsc_y, double bsc_z)
{
  ...
  bool debug = false;
  if (debug)
    std::cout << " enter in VertexFitter_Tk for the Bs decay vertex " << std::endl;

  // if a beam-spot constraint is required :
  TMatrixDSym BeamSpotCovI(3);
  TVectorD BeamSpotPos(3);
  if (BeamSpotConstraint)
  {                                                      // fill in the inverse of the covariance matrix. Convert the units into meters
  ...
  Int_t isel;
  Int_t jsel;                // selected track indices
  ...
  // store the results in an lcio::VertexData object
  // go back from meters to millimeters for the units
  float conv = 1e3;
  std::vector<float> covMatrix; // covMat in lcio is a LOWER-triangle matrix.
  covMatrix.push_back(covX(0, 0) * pow(conv, 2));
  covMatrix.push_back(covX(1, 0) * pow(conv, 2));
  covMatrix.push_back(covX(1, 1) * pow(conv, 2));
  covMatrix.push_back(covX(2, 0) * pow(conv, 2));
  covMatrix.push_back(covX(2, 1) * pow(conv, 2));
  covMatrix.push_back(covX(2, 2) * pow(conv, 2));

  float Ndof = 2.0 * Ntr - 3.0;

  result.setPrimary(Primary);
  result.setChi2(Chi2 / Ndof); // I store the normalised chi2 here

  float vpos[3] = {x(0) * conv, x(1) * conv, x(2) * conv};

  result.setPosition(vpos); // store the  vertex in mm

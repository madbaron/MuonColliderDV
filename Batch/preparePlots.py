...
parser = OptionParser()
parser.add_option("--outDir", help="dir to store the output", default="/nfs/dust/atlas/user/fmeloni/MuonCollider/MuonColliderDV/Batch/scripts_plots/")
...
for ievt in range(int(options.min), int(options.max), int(options.step)):
	...
	filename = "histos_"+str(options.process)+"_"+str(startevent).zfill(6)+".submit"

	fsub = open(options.outDir+filename,"w+")
	
	fsub.write("executable = /nfs/dust/atlas/user/fmeloni/MuonCollider/MuonColliderDV/Batch/PLOTS_Job_EVENT.sh \n")
	...
	if options.nologs:
		...
		fsub.write("output = "+logDir+"plots_"+str(options.process)+"_"+str(startevent).zfill(6)+"_$(ClusterId).$(ProcId).out"+" \n")
		fsub.write("error = "+logDir+"plots_"+str(options.process)+"_"+str(startevent).zfill(6)+"_$(ClusterId).$(ProcId).err"+" \n")
		fsub.write("log = "+logDir+"plots_"+str(options.process)+"_"+str(startevent).zfill(6)+"_$(ClusterId).$(ProcId).log"+" \n")

...
TrackSelector aTrackSelector;

TrackSelector::TrackSelector() : Processor("TrackSelector")
{
...
void TrackSelector::processRunHeader(LCRunHeader *run)
{

    _nRun++;
}

void TrackSelector::processEvent(LCEvent *evt)
{
    ...
    for (size_t itTrack = 0; itTrack < nTracks; itTrack++)
    {
        ...
        if (track->getChi2() / track->getNdf() > 10)
        {
            continue;
        }
...
void TrackSelector::check(LCEvent *evt)
{
    // nothing to check here - could be used to fill checkplots in reconstruction processor
}

void TrackSelector::end()
{
    streamlog_out(DEBUG8) << "TrackSelector::end()  " << name()
                          << " processed " << _nEvt << " events in " << _nRun << " runs "
                          << std::endl;
}

void TrackSelector::getCollection(LCCollection *&collection, std::string collectionName, LCEvent *evt)
{
    try
    {
